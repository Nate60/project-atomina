...
namespace ATMA
{
    ...
    GLRenderContextOpenGLUnixImpl::GLRenderContextOpenGLUnixImpl(): GLRenderContext()
    {
        ...
        XSetErrorHandler([](Display *dsp, XErrorEvent *error){
        ...
        {
            ATMA_ENGINE_WARN("glXCreateContextAttribsARB() not found ... using old-style GLX context");
        ...
        else
        {
            ...
            glXMakeCurrent(m_display, 0L, NULL);
            glXDestroyContext(m_display, m_glContext);
            m_glContext = glXCreateContextAttribsARB(m_display, bestFbc, 0, GL_TRUE, context_attribs);
        }
        glXMakeCurrent(m_display, root, m_glContext);
    }
