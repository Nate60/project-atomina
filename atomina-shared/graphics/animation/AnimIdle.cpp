...
namespace ATMA
{
	AnimIdle::AnimIdle(const std::string &l_name,
		...
		const bool &l_loop
	): AnimBase(l_name, l_width, l_height, l_startFrame,l_endFrame,l_activeStart,l_activeEnd,l_loop)
	{
	...
	AnimIdle::AnimIdle(const std::string &l_name,
		...
		const unsigned int &l_activeEnd,
		const bool &l_loop
	) : AnimBase(l_name,l_dimensions, l_startFrame, l_endFrame, l_activeStart, l_activeEnd, l_loop)
	{
