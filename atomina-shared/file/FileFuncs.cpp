...
namespace ATMA
{
	...
	std::vector<stdfs::path> getDirectoryList(
	...
	)
	{
		...
		auto filtered = std::copy_if(std::make_move_iterator(paths.begin()), std::make_move_iterator(paths.end()), std::back_inserter(filtered_paths), [regex](const stdfs::path &de)
			{
				return std::regex_search(de.string(), regex);
			}
		);
