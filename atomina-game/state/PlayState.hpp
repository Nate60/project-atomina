...
public:
        ...
        BaseState()
    {
        ...
        m_defaultProg->link();
        auto selectedTextID = ctx.m_resMan->registerResource("selected", 0u, "res/selected.png");
        auto unselectedTextID = ctx.m_resMan->registerResource("unselected", 0u, "res/unselected.png");
        auto selectedRockTextID = ctx.m_resMan->registerResource("selected", 0u, "res/selectedRock.png");
        auto unselectedRockTextID = ctx.m_resMan->registerResource("unselected", 0u, "res/unselectedRock.png");
        auto selectedPaperTextID = ctx.m_resMan->registerResource("selected", 0u, "res/selectedPaper.png");
        auto unselectedPaperTextID = ctx.m_resMan->registerResource("unselected", 0u, "res/unselectedPaper.png");
        auto selectedScissorsTextID = ctx.m_resMan->registerResource("selected", 0u, "res/selectedScissors.png");
        auto unselectedScissorsTextID = ctx.m_resMan->registerResource("unselected", 0u, "res/unselectedScissors.png");
        m_font = ctx.m_resMan->loadResource<ATMA::GLTexture>(l_fontID);
        m_selectedTexture = ctx.m_resMan->loadResource<ATMA::GLTexture>(selectedTextID);
        m_unselectedTexture = ctx.m_resMan->loadResource<ATMA::GLTexture>(unselectedTextID);
        m_selectedRockTexture = ctx.m_resMan->loadResource<ATMA::GLTexture>(selectedRockTextID);
        m_unselectedRockTexture = ctx.m_resMan->loadResource<ATMA::GLTexture>(unselectedRockTextID);
        m_selectedPaperTexture = ctx.m_resMan->loadResource<ATMA::GLTexture>(selectedPaperTextID);
        m_unselectedPaperTexture = ctx.m_resMan->loadResource<ATMA::GLTexture>(unselectedPaperTextID);
        m_selectedScissorsTexture = ctx.m_resMan->loadResource<ATMA::GLTexture>(selectedScissorsTextID);
        m_unselectedScissorsTexture = ctx.m_resMan->loadResource<ATMA::GLTexture>(unselectedScissorsTextID);
        for(int i = 0; i < 3; ++i)
        {
            m_buttons[i].m_buttonId = ctx.m_attrMan->createObject(ctx);
            m_buttons[i].m_iconId = ctx.m_attrMan->createObject(ctx);
            m_buttons[i].m_textId = ctx.m_attrMan->createObject(ctx);
        }
        for(int i = 0; i < 2; ++i)
        {
            m_scoreText[i].m_scoreNameId = ctx.m_attrMan->createObject(ctx);
            m_scoreText[i].m_scoreTextId = ctx.m_attrMan->createObject(ctx);
            m_results[i].m_iconId = ctx.m_attrMan->createObject(ctx);
        }
        m_message.m_messageId = ctx.m_attrMan->createObject(ctx);
        m_resultTimer.m_timerId = ctx.m_attrMan->createObject(ctx);
    }
    ...
    virtual void activate() override
    {
        ...
        for(int i = 0; i < 2; ++i)
        {
            ctx.m_attrMan->addAttribute(ctx, m_scoreText[i].m_scoreNameId, ATMA::AttributeType(ATMA::Attribute::Text));
            ctx.m_attrMan->addAttribute(ctx, m_scoreText[i].m_scoreTextId, ATMA::AttributeType(ATMA::Attribute::Text));
            m_scoreText[i].m_scoreName = ctx.m_attrMan->getAttribute<ATMA::AttrText>(
                m_scoreText[i].m_scoreNameId, ATMA::AttributeType(ATMA::Attribute::Text)
            );
            m_scoreText[i].m_scoreText = ctx.m_attrMan->getAttribute<ATMA::AttrText>(
    ...
    virtual void deactivate() override
    {
        ...
        for(int i = 0; i < 2; ++i)
        {
            ctx.m_attrMan->removeAttribute(
                ctx, m_scoreText[i].m_scoreNameId, ATMA::AttributeType(ATMA::Attribute::Text)
            );
            ctx.m_attrMan->removeAttribute(
                ctx, m_scoreText[i].m_scoreTextId, ATMA::AttributeType(ATMA::Attribute::Text)
            );
            if(ctx.m_attrMan->hasAttribute(m_results[i].m_iconId, ATMA::AttributeType(ATMA::Attribute::Render)))
                ctx.m_attrMan->removeAttribute(
                    ctx, m_results[i].m_iconId, ATMA::AttributeType(ATMA::Attribute::Render)
                );
        }
        if(ctx.m_attrMan->hasAttribute(m_message.m_messageId, ATMA::AttributeType(ATMA::Attribute::Text)))
            ctx.m_attrMan->removeAttribute(ctx, m_message.m_messageId, ATMA::AttributeType(ATMA::Attribute::Text));
        if(ctx.m_attrMan->hasAttribute(m_resultTimer.m_timerId, GameAttributeType(GameAttributeEnum::TIMER)))
            ctx.m_attrMan->removeAttribute(ctx, m_resultTimer.m_timerId, GameAttributeType(GameAttributeEnum::TIMER));
    }
                ...
                case static_cast<unsigned int>(GameNetMessageEnum::PLAYER_LOSE):
                    {
                        ...
                        for(int i = 0; i < 2; ++i)
                        {
                            ctx.m_attrMan->addAttribute(
                                ctx, m_results[i].m_iconId, ATMA::AttributeType(ATMA::Attribute::Render)
                            );
                            m_results[i].m_icon = ctx.m_attrMan->getAttribute<ATMA::AttrRenderable>(
                        ...
                        ctx.m_attrMan->addAttribute(
                            ctx, m_resultTimer.m_timerId, GameAttributeType(GameAttributeEnum::TIMER)
                        );
                        m_resultTimer.m_timer = ctx.m_attrMan->getAttribute<AttrTimer>(
        ...
        case static_cast<unsigned int>(GameEventEnum::TIMER_COMPLETE):
            {
                ...
                if(ctx.m_attrMan->hasAttribute(m_message.m_messageId, ATMA::AttributeType(ATMA::Attribute::Text)))
                    ctx.m_attrMan->removeAttribute(
                        ctx, m_message.m_messageId, ATMA::AttributeType(ATMA::Attribute::Text)
                    );
                if(ctx.m_attrMan->hasAttribute(m_resultTimer.m_timerId, GameAttributeType(GameAttributeEnum::TIMER)))
                    ctx.m_attrMan->removeAttribute(
                        ctx, m_resultTimer.m_timerId, GameAttributeType(GameAttributeEnum::TIMER)
                ...
                case ATMA::KeyEnum::RETURN:
                    {
                        ...
                        m_message.m_messageText = ctx.m_attrMan->getAttribute<ATMA::AttrText>(
...
private:
    void addButtons()
    {
        for(int i = 0; i < 3; i++)
        {
            ctx.m_attrMan->addAttribute(ctx, m_buttons[i].m_buttonId, ATMA::AttributeType(ATMA::Attribute::Render));
            ctx.m_attrMan->addAttribute(ctx, m_buttons[i].m_iconId, ATMA::AttributeType(ATMA::Attribute::Render));
            ctx.m_attrMan->addAttribute(ctx, m_buttons[i].m_textId, ATMA::AttributeType(ATMA::Attribute::Text));
            m_buttons[i].m_button = ctx.m_attrMan->getAttribute<ATMA::AttrRenderable>(
                m_buttons[i].m_buttonId, ATMA::AttributeType(ATMA::Attribute::Render)
            );
            m_buttons[i].m_icon = ctx.m_attrMan->getAttribute<ATMA::AttrRenderable>(
                m_buttons[i].m_iconId, ATMA::AttributeType(ATMA::Attribute::Render)
            );
            m_buttons[i].m_text = ctx.m_attrMan->getAttribute<ATMA::AttrText>(
                m_buttons[i].m_textId, ATMA::AttributeType(ATMA::Attribute::Text)
            );
    ...
    void removeButtons()
    {
        for(int i = 0; i < 3; i++)
        {
            ctx.m_attrMan->removeAttribute(ctx, m_buttons[i].m_buttonId, ATMA::AttributeType(ATMA::Attribute::Render));
            ctx.m_attrMan->removeAttribute(ctx, m_buttons[i].m_iconId, ATMA::AttributeType(ATMA::Attribute::Render));
            ctx.m_attrMan->removeAttribute(ctx, m_buttons[i].m_textId, ATMA::AttributeType(ATMA::Attribute::Text));
        }
    }
