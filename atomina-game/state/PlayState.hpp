...
public:
        ...
        BaseState(l_ctx)
    {
        ...
        m_defaultProg->link();
        auto selectedTextID = l_ctx->m_resMan->registerResource("selected", 0u, "res/selected.png");
        auto unselectedTextID = l_ctx->m_resMan->registerResource("unselected", 0u, "res/unselected.png");
        auto selectedRockTextID = l_ctx->m_resMan->registerResource("selected", 0u, "res/selectedRock.png");
        auto unselectedRockTextID = l_ctx->m_resMan->registerResource("unselected", 0u, "res/unselectedRock.png");
        auto selectedPaperTextID = l_ctx->m_resMan->registerResource("selected", 0u, "res/selectedPaper.png");
        auto unselectedPaperTextID = l_ctx->m_resMan->registerResource("unselected", 0u, "res/unselectedPaper.png");
        auto selectedScissorsTextID = l_ctx->m_resMan->registerResource("selected", 0u, "res/selectedScissors.png");
        auto unselectedScissorsTextID =
            l_ctx->m_resMan->registerResource("unselected", 0u, "res/unselectedScissors.png");
        m_font = l_ctx->m_resMan->loadResource<ATMA::GLTexture>(l_fontID);
        m_selectedTexture = l_ctx->m_resMan->loadResource<ATMA::GLTexture>(selectedTextID);
        m_unselectedTexture = l_ctx->m_resMan->loadResource<ATMA::GLTexture>(unselectedTextID);
        m_selectedRockTexture = l_ctx->m_resMan->loadResource<ATMA::GLTexture>(selectedRockTextID);
        m_unselectedRockTexture = l_ctx->m_resMan->loadResource<ATMA::GLTexture>(unselectedRockTextID);
        m_selectedPaperTexture = l_ctx->m_resMan->loadResource<ATMA::GLTexture>(selectedPaperTextID);
        m_unselectedPaperTexture = l_ctx->m_resMan->loadResource<ATMA::GLTexture>(unselectedPaperTextID);
        m_selectedScissorsTexture = l_ctx->m_resMan->loadResource<ATMA::GLTexture>(selectedScissorsTextID);
        m_unselectedScissorsTexture = l_ctx->m_resMan->loadResource<ATMA::GLTexture>(unselectedScissorsTextID);
        for(int i = 0; i < 3; ++i)
        {
            m_buttons[i].m_buttonId = l_ctx->m_attrMan->createObject();
            m_buttons[i].m_iconId = l_ctx->m_attrMan->createObject();
            m_buttons[i].m_textId = l_ctx->m_attrMan->createObject();
        }
        for(int i = 0; i < 2; ++i)
        {
            m_scoreText[i].m_scoreNameId = l_ctx->m_attrMan->createObject();
            m_scoreText[i].m_scoreTextId = l_ctx->m_attrMan->createObject();
            m_results[i].m_iconId = l_ctx->m_attrMan->createObject();
        }
        m_message.m_messageId = l_ctx->m_attrMan->createObject();
        m_resultTimer.m_timerId = l_ctx->m_attrMan->createObject();
    }
    ...
    virtual void activate(ATMA::ATMAContext *l_ctx) override
    {
        ...
        for(int i = 0; i < 2; ++i)
        {
            l_ctx->m_attrMan->addAttribute(
                l_ctx, m_scoreText[i].m_scoreNameId, ATMA::AttributeType(ATMA::Attribute::Text)
            );
            l_ctx->m_attrMan->addAttribute(
                l_ctx, m_scoreText[i].m_scoreTextId, ATMA::AttributeType(ATMA::Attribute::Text)
            );
            m_scoreText[i].m_scoreName = l_ctx->m_attrMan->getAttribute<ATMA::AttrText>(
                m_scoreText[i].m_scoreNameId, ATMA::AttributeType(ATMA::Attribute::Text)
            );
            m_scoreText[i].m_scoreText = l_ctx->m_attrMan->getAttribute<ATMA::AttrText>(
    ...
    virtual void deactivate(ATMA::ATMAContext *l_ctx) override
    {
        ...
        for(int i = 0; i < 2; ++i)
        {
            ...
            if(l_ctx->m_attrMan->hasAttribute(m_results[i].m_iconId, ATMA::AttributeType(ATMA::Attribute::Render)))
                l_ctx->m_attrMan->removeAttribute(
                    l_ctx, m_results[i].m_iconId, ATMA::AttributeType(ATMA::Attribute::Render)
                );
        }
        if(l_ctx->m_attrMan->hasAttribute(m_message.m_messageId, ATMA::AttributeType(ATMA::Attribute::Text)))
            l_ctx->m_attrMan->removeAttribute(l_ctx, m_message.m_messageId, ATMA::AttributeType(ATMA::Attribute::Text));
        if(l_ctx->m_attrMan->hasAttribute(m_resultTimer.m_timerId, GameAttributeType(GameAttributeEnum::TIMER)))
            l_ctx->m_attrMan->removeAttribute(
                l_ctx, m_resultTimer.m_timerId, GameAttributeType(GameAttributeEnum::TIMER)
            );
    }
                ...
                case static_cast<unsigned int>(GameNetMessageEnum::PLAYER_LOSE):
                    {
                        ...
                        for(int i = 0; i < 2; ++i)
                        {
                            ...
                            m_results[i].m_icon = l_ctx->m_attrMan->getAttribute<ATMA::AttrRenderable>(
                        ...
                        m_resultTimer.m_timer = l_ctx->m_attrMan->getAttribute<AttrTimer>(
        ...
        case static_cast<unsigned int>(GameEventEnum::TIMER_COMPLETE):
            {
                ...
                if(l_ctx->m_attrMan->hasAttribute(m_message.m_messageId, ATMA::AttributeType(ATMA::Attribute::Text)))
                    l_ctx->m_attrMan->removeAttribute(
                        l_ctx, m_message.m_messageId, ATMA::AttributeType(ATMA::Attribute::Text)
                    );
                if(l_ctx->m_attrMan->hasAttribute(m_resultTimer.m_timerId, GameAttributeType(GameAttributeEnum::TIMER)))
                    l_ctx->m_attrMan->removeAttribute(
                        l_ctx, m_resultTimer.m_timerId, GameAttributeType(GameAttributeEnum::TIMER)
                ...
                case ATMA::KeyEnum::RETURN:
                    {
                        ...
                        m_message.m_messageText = l_ctx->m_attrMan->getAttribute<ATMA::AttrText>(
...
private:
    void addButtons(ATMA::ATMAContext *l_ctx)
    {
        for(int i = 0; i < 3; i++)
        {
            l_ctx->m_attrMan->addAttribute(
                l_ctx, m_buttons[i].m_buttonId, ATMA::AttributeType(ATMA::Attribute::Render)
            );
            l_ctx->m_attrMan->addAttribute(l_ctx, m_buttons[i].m_iconId, ATMA::AttributeType(ATMA::Attribute::Render));
            l_ctx->m_attrMan->addAttribute(l_ctx, m_buttons[i].m_textId, ATMA::AttributeType(ATMA::Attribute::Text));
            m_buttons[i].m_button = l_ctx->m_attrMan->getAttribute<ATMA::AttrRenderable>(
                m_buttons[i].m_buttonId, ATMA::AttributeType(ATMA::Attribute::Render)
            );
            m_buttons[i].m_icon = l_ctx->m_attrMan->getAttribute<ATMA::AttrRenderable>(
                m_buttons[i].m_iconId, ATMA::AttributeType(ATMA::Attribute::Render)
            );
            m_buttons[i].m_text = l_ctx->m_attrMan->getAttribute<ATMA::AttrText>(
    ...
    void removeButtons(ATMA::ATMAContext *l_ctx)
    {
        for(int i = 0; i < 3; i++)
        {
            l_ctx->m_attrMan->removeAttribute(
            ...
            l_ctx->m_attrMan->removeAttribute(
                l_ctx, m_buttons[i].m_iconId, ATMA::AttributeType(ATMA::Attribute::Render)
            );
            l_ctx->m_attrMan->removeAttribute(l_ctx, m_buttons[i].m_textId, ATMA::AttributeType(ATMA::Attribute::Text));
        }
    }
