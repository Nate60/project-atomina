...
public:
        ...
        BaseState(l_ctx)
    {
        ...
        for(int i = 0; i < 3; i++)
        {
            m_menuOpts[i].first = l_ctx->m_attrMan->createObject();
            m_menuOpts[i].second = l_ctx->m_attrMan->createObject();
        }
    }
    ...
    virtual void activate(ATMA::ATMAContext *l_ctx) override
    {
        ...
        for(int i = 0; i < 3; i++)
        {
            l_ctx->m_attrMan->addAttribute(l_ctx, m_menuOpts[i].first, ATMA::AttributeType(ATMA::Attribute::Render));
            l_ctx->m_attrMan->addAttribute(l_ctx, m_menuOpts[i].second, ATMA::AttributeType(ATMA::Attribute::Text));
            m_menuObjs[i].first = l_ctx->m_attrMan->getAttribute<ATMA::AttrRenderable>(
                m_menuOpts[i].first, ATMA::AttributeType(ATMA::Attribute::Render)
            );
            m_menuObjs[i].second = l_ctx->m_attrMan->getAttribute<ATMA::AttrText>(
    ...
    virtual void deactivate(ATMA::ATMAContext *l_ctx) override
    {
        ...
        for(int i = 0; i < 3; i++)
        {
            l_ctx->m_attrMan->removeAttribute(l_ctx, m_menuOpts[i].first, ATMA::AttributeType(ATMA::Attribute::Render));
            l_ctx->m_attrMan->removeAttribute(l_ctx, m_menuOpts[i].second, ATMA::AttributeType(ATMA::Attribute::Text));
        }
    }
        ...
        case ATMA::WindowEventEnum::KeyDowned:
            {
                ...
                case ATMA::KeyEnum::RETURN:
                    ...
                    case 1:
                        break;
                    case 2:
                        l_ctx->m_stateMan->switchToState(l_ctx, GameStateType(GameStateEnum::LOBBY));
                        break;
                    }
                    break;
                }
