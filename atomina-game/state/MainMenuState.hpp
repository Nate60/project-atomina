...
public:
        ...
        BaseState()
    {
        ...
        for(int i = 0; i < 3; i++)
        {
            m_menuOpts[i].first = ctx.m_attrMan->createObject(ctx);
            m_menuOpts[i].second = ctx.m_attrMan->createObject(ctx);
        }
    }
    ...
    virtual void activate() override
    {
        ...
        for(int i = 0; i < 3; i++)
        {
            ctx.m_attrMan->addAttribute(ctx, m_menuOpts[i].first, ATMA::AttributeType(ATMA::Attribute::Render));
            ctx.m_attrMan->addAttribute(ctx, m_menuOpts[i].second, ATMA::AttributeType(ATMA::Attribute::Text));
            m_menuObjs[i].first = ctx.m_attrMan->getAttribute<ATMA::AttrRenderable>(
                m_menuOpts[i].first, ATMA::AttributeType(ATMA::Attribute::Render)
            );
            m_menuObjs[i].second = ctx.m_attrMan->getAttribute<ATMA::AttrText>(
    ...
    virtual void deactivate() override
    {
        ...
        for(int i = 0; i < 3; i++)
        {
            ctx.m_attrMan->removeAttribute(ctx, m_menuOpts[i].first, ATMA::AttributeType(ATMA::Attribute::Render));
            ctx.m_attrMan->removeAttribute(ctx, m_menuOpts[i].second, ATMA::AttributeType(ATMA::Attribute::Text));
        }
    }
        ...
        case ATMA::WindowEventEnum::KeyDowned:
            {
                ...
                case ATMA::KeyEnum::RETURN:
                    ...
                    case 1:
                        break;
                    case 2:
                        ctx.m_stateMan->switchToState(GameStateType(GameStateEnum::LOBBY));
                        break;
                    }
                    break;
                }
