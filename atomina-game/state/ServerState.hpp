...
{
public:
    ServerState(ATMA::ATMAContext *l_ctx): BaseState(l_ctx) {}
        ...
        case static_cast<unsigned int>(ATMA::NetworkMessageEnum::PORT_REQUEST):
            {
                ...
                for(; i < m_ports.size(); i++)
                {
                    ...
                    l_ctx->m_netMan->sendMessage(broadcast, m_ports[i]);
                }
                ...
                l_ctx->m_netMan->sendMessage(resp, connId);
            }
            break;
        case static_cast<unsigned int>(ATMA::NetworkMessageEnum::STATE_CHANGE):
            {
                ...
                l_ctx->m_netMan->broadcastMessage(nm);
            }
            break;
                    ...
                    case -1:
                        {
                            ...
                            l_ctx->m_netMan->sendMessage(nm, m_playerChoices[0].first);
                            nm = ATMA::NetworkMessage{GameNetMessageType(GameNetMessageEnum::PLAYER_WIN), right};
                            l_ctx->m_netMan->sendMessage(nm, m_playerChoices[1].first);
                        }
                        break;
                    case 0:
                        {
