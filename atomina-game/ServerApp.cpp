...
void ServerApp::setup(ATMA::ATMAContext *l_ctx)
{
    ...
    std::shared_ptr<ServerState> server = std::make_shared<ServerState>(l_ctx);
    l_ctx->m_eventMan->addObjectEventListener(ATMA::ObjectEventType(ATMA::ObjectEvent::Network), server);
    l_ctx->m_stateMan->addState(l_ctx, GameStateType(GameStateEnum::PLAYSTATE), server);
    l_ctx->m_netMan->startHosting(l_ctx, 4734);
}
