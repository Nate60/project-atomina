...
void GameApp::setup(ATMA::ATMAContext *l_ctx)
{
    ...
    l_ctx->m_sysMan->addSystemType<SysConnection>(l_ctx, GameSystemType(GameSystemEnum::CONNECTION));
    l_ctx->m_attrMan->registerAttributeType<AttrTimer>(GameAttributeType(GameAttributeEnum::TIMER));
    l_ctx->m_sysMan->addSystemType<SysTime>(l_ctx, GameSystemType(GameSystemEnum::TIME));
    ...
    std::shared_ptr<MainMenuState> state =
        std::make_shared<MainMenuState>(l_ctx, m_win, vertShaderID, fragShaderID, fontID);
    l_ctx->m_stateMan->addState(l_ctx, GameStateType(GameStateEnum::MAINMENU), std::move(state));
    std::shared_ptr<LobbyState> lobby = std::make_shared<LobbyState>(l_ctx, m_win, vertShaderID, fragShaderID, fontID);
    l_ctx->m_eventMan->addObjectEventListener(ATMA::ObjectEventType(ATMA::ObjectEvent::Network), lobby);
    l_ctx->m_stateMan->addState(l_ctx, GameStateType(GameStateEnum::LOBBY), std::move(lobby));
    std::shared_ptr<PlayState> play = std::make_shared<PlayState>(l_ctx, m_win, vertShaderID, fragShaderID, fontID);
    ...
    l_ctx->m_stateMan->addState(l_ctx, GameStateType(GameStateEnum::PLAYSTATE), std::move(play));
    ...
    auto channelId = l_ctx->m_audioMan->createAudioChannel(l_ctx);
